...
const STAGE_DETAILS = {
  ...
  stage3_accessibility_critique: {
    ...
    provider: 'OpenAI Vision / Google Gemini',
    model: 'gpt-4o-mini | gpt-5.4 | gemini-3-flash | gemini-3-pro',
    inputs: ['stage2/stage3 source image', 'AAC simplicity check'],
    outputs: ['simplicity_ok', 'issues', 'correction_recommendations', 'simplicity_problem'],
    instruction: 'You are an AAC accessibility reviewer for children and users with special needs. Return STRICT JSON with keys {"simplicity_ok":"yes|no", "issues":"...", "correction_recommendations":"...", "simplicity_problem":"none|busy_scene|too_many_objects|distracting_background|unclear_focus|visual_overload"}.',
    requestExample: {
      content: [
        { type: 'text', text: 'Check that the image is simple, not visually busy, and easy for AAC users to understand.' },
        { type: 'image_url', image_url: { url: '<data:image/...>' } },
      ],
    },
  },
...
export default function AlgorithmStaticMap({ assistantName = '', config = null, mode = 'csv_dag' }) {
  ...
  const csvDagNodes = useMemo(
    () => [
      ...
      { id: 'stage3_accessibility_critique', label: 'Stage 3.16 Simplicity Critique', subtitle: 'AAC clarity + low clutter check', status: 'queued', x: 1240, y: 72 },
      ...
      { id: 'stage4_background', label: 'Stage 4 White Background', subtitle: 'base winner white BG', status: 'queued', x: 2440, y: 72 },
      {
  ...
  const csvDagEdges = useMemo(
    () => [
      ...
      { from: 'stage3_critique', to: 'stage3_accessibility_critique', label: 'always add simplicity review', fromPort: 'bottom', toPort: 'top' },
      { from: 'stage3_anatomy_critique', to: 'stage3_accessibility_critique', label: 'include anatomy fixes first', fromPort: 'right', toPort: 'left' },
      { from: 'stage3_accessibility_critique', to: 'stage3_prompt_upgrade', label: 'include simplicity fixes', fromPort: 'right', toPort: 'left' },
  ...
  const legacyNodes = useMemo(
    () => [
      ...
      { id: 'stage5_variant_white_bg', label: 'Stage 9 Variant White BG', subtitle: 'white background for every final variant', badge: 'backend: stage5_variant_white_bg', status: 'queued', x: 3940, y: 72 },
      { id: 'completed_pass', label: 'Completed Pass', subtitle: 'ready for export', status: 'ok', x: 4240, y: 72 },
