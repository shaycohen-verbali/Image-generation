...
COLUMN_ALIASES = {
    ...
    "category": ["category"],
    "sense_id": ["sense id", "sense_id"],
    "context": ["context"],
    "word_synonyms_for_better_meaning": ["word synonyms for better meaning", "word_synonyms_for_better_meaning", "synonyms"],
...
def parse_entries_csv(content: bytes) -> list[dict[str, str]]:
    ...
    for row in reader:
        parsed = {
            ...
            "sense_id": _pick(row, COLUMN_ALIASES["sense_id"]),
            "context": _pick(row, COLUMN_ALIASES["context"]),
            "word_synonyms_for_better_meaning": _pick(row, COLUMN_ALIASES["word_synonyms_for_better_meaning"]),
