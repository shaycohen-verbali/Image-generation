...
def test_update_runtime_config_clamps_worker_count(db_session) -> None:
    ...
    assert config_high.max_parallel_runs == 999
...
def test_update_runtime_config_clamps_variant_worker_count(db_session) -> None:
    ...
    assert config_high.max_variant_workers == 999
...
def test_update_runtime_config_normalizes_model_fields(db_session) -> None:
    ...
    assert config.quality_gate_model == "gemini-3.1-pro-preview"
    ...
    config = repo.update_runtime_config(
        {
            ...
            "quality_gate_model": "gemini-3.1-flash-lite",
        }
    )

    assert config.stage3_critique_model == "gemini-3.1-pro-preview"
    assert config.stage3_anatomy_critique_model == "gemini-3-flash-preview"
    assert config.quality_gate_model == "gemini-3.1-flash-lite"
...
def test_add_asset_is_idempotent_by_run_stage_attempt_and_file_name(db_session) -> None:
    ...
    first = repo.add_asset(
        ...
        attempt=1,
        file_name="soccer__verb__reg__m__kid__w__sense-id.jpg",
